...
- A stack is **only** touched if it has `override.tf` (local backend). Any stack with `backend.hcl` (real S3 backend) is **always skipped** — fmt/init/validate/plan/apply never run against it through this driver.
...
- **Backend is currently local** via a gitignored `override.tf` (real state, but disposable) because the real S3 backend bucket doesn't exist yet; `backend.tf` only has partial S3 config, injected via `-backend-config=backend.hcl` (also gitignored) once that bucket exists. This is exactly the `override.tf` vs `backend.hcl` distinction the `terraform-deploy` skill driver uses to decide whether to touch a stack at all.
